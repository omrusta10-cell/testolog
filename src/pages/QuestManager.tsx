...
export default function QuestManager() {
  ...
  const fetchQuests = async () => {
    setLoading(true);
    try {
      const res = await api.get("/api/db/data?table=quest", {
        headers: { "x-db-name-override": "player" }
      });
      setQuests(res.data);
    } catch (err: any) {
      toast.error("Quest verileri yüklenemedi: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" onClick={fetchQuests} disabled={loading} className="gap-2">
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="relative w-72">
              ...
              <Input
                placeholder="Quest adı veya Oyuncu ID..."
                value={search}
            ...
            <Table>
              ...
              <TableBody>
                {filteredQuests.map((quest, i) => (
                  <TableRow key={i}>
                    <TableCell className="font-mono text-xs">{quest.dwPID}</TableCell>
